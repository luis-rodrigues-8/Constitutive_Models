# Here 5 lists of parameters will be generated linearly, one for each of the params, between two limit values.
# Then the permutations of these 5 lists will be computed and appended to a list of permutations "params".
# Noise will be then added to the values, while assuring that all the values are physically acceptable.

import os
os.environ["MODIN_CPUS"] = "24" #max cpus. important to avoid overthreading
...
c_list = np.round(np.linspace(c_min, c_max, n_c), decimals)
κ_list = np.round(np.linspace(κ_min, κ_max, n_κ), decimals)
k1_list = np.round(np.linspace(k1_min, k1_max, n_k1), decimals)
k2_list = np.round(np.linspace(k2_min, k2_max, n_k2), decimals)
θ_list = np.round(np.linspace(θ_min, θ_max, n_θ), decimals)
    ...
    for j in κ_list:
        for k in k1_list:
            for l in k2_list:
                for m in θ_list:
                    while True:
                        i_noise = i + noise * (np.random.random() - 0.5) * (c_max - c_min)
                        j_noise = j + noise * (np.random.random() - 0.5) * (κ_max - κ_min)
                        k_noise = k + noise * (np.random.random() - 0.5) * (k1_max - k1_min)
                        l_noise = l + noise * (np.random.random() - 0.5) * (k2_max - k2_min)
                        m_noise = m + noise * (np.random.random() - 0.5) * (θ_max - θ_min)
                        if (i_noise>0 and (j_noise>=0.0 and j_noise<=1/3) and k_noise>0 
                                            and l_noise>0 and (m_noise>=0 and m_noise<=90)):
                            break
                            
                    params.append([i_noise,j_noise,k_noise,l_noise,m_noise])
...
np.save('X_run_10_noise', X)
np.save('y_run_10_noise', y)
